...
#problem I had with this is I did not use attr_accessor for :beers or
class BeerSong
  attr_accessor :beers #setting and getting beers

  def initialize(beers)#initalizing beers and calling beers 0 if beers are less
    beers = 0  if beers < 0
    beers = 99 if beers > 99 #and the max beers can be is 99
    self.beers = beers
  end

  def print_song
    beers.downto 1 do |i| #iterate through how many beers there are given down to one and print the stanza next
      print_stanza i
    end
  end
